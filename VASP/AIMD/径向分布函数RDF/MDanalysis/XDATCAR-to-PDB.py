        ...
        for i in range(len(self.total_elements)):
            tobewriten.append('%4s%7d%4s%5s%6d%4s%8.3f%8.3f%8.3f%6.2f%6.2f%12s' \
                              % ("ATOM", i + 1, self.total_elements[i], "MOL", 1, '    ', self.cartesian_position[i][0],
                                 self.cartesian_position[i][1], self.cartesian_position[i][2], 1.0, 0.0,
                                 self.total_elements[i]))
...
class plot(object):
    ...
    def __init__(self, lwd, font, dpi, figsize, XDATCAR_inst=None):
        self.XDATCAR_inst = XDATCAR_inst;
        self.timestep = self.XDATCAR_inst.timestep
    ...
    def plotfigure(self, title='MD temperature and energy profile'):
        from matplotlib import pyplot as plt
        self.newtemp = self.XDATCAR_inst.temp;
        self.newenergy = self.XDATCAR_inst.diffusion_coefficient
...
if __name__ == "__main__":
    ...
    if options.end == 'false':
        ...
        XDATCAR_inst(
            't>= %r and t <= %r' % (int(options.begin), int(options.end)))  # frame 10~300  corresponding to 20~600fs
                    ...
                    if options.periodic == True:
                        if i == XDATCAR_inst.lowrange:
                            ...
                            if int(options.index) != -1:
                                real_atomic_cartesian = XDATCAR_inst.reset_cartesian(real_atomic_cartesian,
                                                                                     int(options.index) - 1)
    ...
    print('Selected time-range:{0}~{1}fs'.format((XDATCAR_inst.lowrange) * timestep, (XDATCAR_inst.uprange) * timestep))
